...
!  **** 1D FAWA equation in the vertical ****
  ...
  integer,parameter :: years=1000,ry=1000, kmax = 501,myear = 178560 , mmax = myear*ry+1,mout = 558*2, msize = 160000, mera = myear/992  !!!!!!!!!! dt = 120
  ...
  ! 6 alpha, 7 const1, 8 ppow, 9 aexp,
  ! 10 tag, 11 forcing_file, 12 output_dir
  ...
  character(len=64)  :: arg1, arg2, arg3, arg4, arg5
  character(len=64)  :: arg6, arg7, arg8, arg9
  character(len=256) :: arg10
  character(len=512) :: arg11, arg12

  character(len=512) :: forcing_file, output_dir
  character(len=512) :: out_a, out_u, out_ur, out_fz, out_fzn
  character(len=512) :: out_abudget, out_ubudget
  ...
  ! Default forcing file and output directory
  forcing_file = '/scratch/smliu01/1d_model/climatechange/forcing/ctl/forcing_total_1000_ctl.nc'
  output_dir   = '.'
...
! 10 tag, 11 forcing_file, 12 output_dir
  ...
  call get_command_argument(11, arg11, length=arglen)
  if (arglen > 0) forcing_file = trim(arg11)

  call get_command_argument(12, arg12, length=arglen)
  if (arglen > 0) output_dir = trim(arg12)
  ...
  out_a   = trim(output_dir)//'/aout'  //trim(fname)//'.nc'
  out_u   = trim(output_dir)//'/uout'  //trim(fname)//'.nc'
  out_ur  = trim(output_dir)//'/urout' //trim(fname)//'.nc'
  out_fz  = trim(output_dir)//'/fzout' //trim(fname)//'.nc'
  out_fzn = trim(output_dir)//'/fznout'//trim(fname)//'.nc'

  out_abudget = trim(output_dir)//'/abudget'//trim(fname)//'.nc'
  out_ubudget = trim(output_dir)//'/ubudget'//trim(fname)//'.nc'

  write(*,*) '================================================='
  write(*,*) 'beta        = ', beta
  write(*,*) 's_tau       = ', s_tau
  write(*,*) 'damp1_days  = ', damp1_days
  write(*,*) 'u_thres     = ', u_thres
  write(*,*) 'flux_extra  = ', flux_extra
  write(*,*) 'alpha       = ', alpha
  write(*,*) 'const1      = ', const1
  write(*,*) 'ppow        = ', ppow
  write(*,*) 'aexp        = ', aexp
  write(*,*) 'outtag      = ', trim(outtag)
  write(*,*) 'forcing_file= ', trim(forcing_file)
  ...
  status = nf90_open(trim(forcing_file), nf90_nowrite, ncid)
  if (status /= nf90_noerr) then
    write(*,*) 'ERROR opening forcing file: ', trim(forcing_file)
    write(*,*) trim(nf90_strerror(status))
    stop 1
  end if

  status = nf90_inq_varid(ncid, "forcing", varID)
  if (status /= nf90_noerr) then
    write(*,*) 'ERROR finding variable "forcing" in: ', trim(forcing_file)
    write(*,*) trim(nf90_strerror(status))
    stop 1
  end if
  ...
  if (status /= nf90_noerr) then
    write(*,*) 'ERROR reading variable "forcing" from: ', trim(forcing_file)
    write(*,*) trim(nf90_strerror(status))
    stop 1
  end if
  ...
  ! --- fawa_interp input (configurable via environment variable) ---
  call get_environment_variable('FAWA_INTERP_FILE', arg11, length=arglen)
  if (arglen == 0) arg11 = 'data/fawa_interp.nc'
  status = nf90_open(trim(arg11),  nf90_nowrite, ncid)
  status = nf90_inq_varid(ncid,"fawa_interp",varID)
  status = nf90_get_var(ncid,varID,fawa_interp)
  status = nf90_close(ncid)

  ! --- urad input (configurable via environment variable) ---
  call get_environment_variable('URAD_FILE', arg11, length=arglen)
  if (arglen == 0) arg11 = 'data/urad_below32_fit_above32_HMshear_501_1000.nc'
  status = nf90_open(trim(arg11),  nf90_nowrite, ncid)
  status = nf90_inq_varid(ncid,"urad",varID)
  status = nf90_get_var(ncid,varID,urad)
  status = nf90_close(ncid)
  
  ! --- init input (configurable via environment variable) ---
  call get_environment_variable('INIT_FILE', arg11, length=arglen)
  if (arglen == 0) arg11 = 'data/init_501_1000.nc'
  status = nf90_open(trim(arg11),  nf90_nowrite, ncid)
...
!   **** main loop starts here ****
  ...
  write(*,*) 'About to write output files:'
  write(*,*) trim(out_a)
  write(*,*) trim(out_u)
  write(*,*) trim(out_ur)
  write(*,*) trim(out_fz)
  ...
  status = nf90_create(trim(out_a), nf90_noclobber, ncid2)
  ...
  status = nf90_open(trim(out_a), nf90_nowrite, ncid)
  ...
  status = nf90_create(trim(out_u), nf90_noclobber, ncid2)
  status = nf90_def_dim(ncid2,"time",msize,it)
  status = nf90_def_dim(ncid2,"height",kmax,iz)
  ...
  status = nf90_open(trim(out_u), nf90_nowrite, ncid)
  status = nf90_inquire(ncid,nDim,nVar,nAtt,uDimID)
  ...
  status = nf90_create(trim(out_ur), nf90_noclobber, ncid2)
  status = nf90_def_dim(ncid2,"time",msize,it)
  status = nf90_def_dim(ncid2,"height",kmax,iz)
  ...
  status = nf90_open(trim(out_ur), nf90_nowrite, ncid)
  status = nf90_inquire(ncid,nDim,nVar,nAtt,uDimID)
  status = nf90_inq_varid(ncid,"uref",varID)
  status = nf90_get_var(ncid,varID,v)
  status = nf90_close(ncid)
  ...
  status = nf90_create(trim(out_fz), nf90_noclobber, ncid2)
  status = nf90_def_dim(ncid2,"time",msize,it)
  status = nf90_def_dim(ncid2,"height",kmax,iz)
  ...
  status = nf90_open(trim(out_fz), nf90_nowrite, ncid)
  status = nf90_inquire(ncid,nDim,nVar,nAtt,uDimID)
  status = nf90_inq_varid(ncid,"fz",varID)
  status = nf90_get_var(ncid,varID,v)
  status = nf90_close(ncid)

  status = nf90_create(trim(out_fzn), nf90_noclobber, ncid2)
  status = nf90_def_dim(ncid2,"time",msize,it)
  status = nf90_def_dim(ncid2,"height",kmax,iz)
  ...
  status = nf90_open(trim(out_fzn), nf90_nowrite, ncid)
  status = nf90_inquire(ncid,nDim,nVar,nAtt,uDimID)
